...
import { jogo, DADOS_PROCESSAMENTO, mostrarAviso, obterBuffRaca } from '../jogo.js'; // Importe DADOS_ESTUDO

const equipeExpandida = ref(false); // Começa "true" (Aberto)
// Verifica se tem QUALQUER slot com item sendo estudado
const temPesquisaAtiva = computed(() => jogo.processamento.some(s => s.item !== null));
// --- ESTADO LOCAL ---
...
if (!jogo.alocacaoCamaraProcessamento) jogo.alocacaoCamaraProcessamento = [null, null, null];
if (!jogo.processamento) jogo.processamento = [{ item: null }];
...
const velocidadeEstudo = computed(() => {
    let total = 0;
    jogo.alocacaoCamaraProcessamento.forEach(id => {
        if (id) {
...
const academicosDisponiveis = computed(() => {
    return jogo.funcionarios.filter(f => 
        ...
        !jogo.alocacaoCamaraProcessamento.includes(f.id)
    );
});
...
const alocarFunc = (func) => {
    jogo.alocacaoCamaraProcessamento[slotFuncionarioSelecionado.value] = func.id;
...
const desalocarFunc = (index) => {
    ...
    jogo.alocacaoCamaraProcessamento[index] = null;
};
const getFuncionario = (index) => {
    const id = jogo.alocacaoCamaraProcessamento[index];
...
const abrirSelecaoItem = (index) => {
    ...
    if (jogo.processamento[index].item) return; 
...
const iniciarEstudo = (itemObj) => {
    ...
    const index = jogo.processamento.findIndex(s => !s.item);
...
<div class="mythic-container animacao-entrada">
    <div class="header-titulo-aba">
        <div class="titulo-nivel">
            <h2>🗡️ Camara de Processamento</h2>
        </div>
    ...
    <div class="painel-topo">
        ...
        <div v-for="(slot, index) in jogo.alocacaoCamaraProcessamento" :key="index" class="slot-wrapper">
    ...
    <div class="mesa-estudos background-acougue">        
        <div class="area-trabalho-central">
             <div class="slot-processamento-ativo">
                <div v-if="!jogo.processamento[0].item" class="mesa-vazia" @click="abrirSelecaoItem(0)">
                    <span class="texto-clique">Processar Carcaça</span>
                </div>

                <div v-else class="processando-ativo">
                    <div class="header-proc">
                        <button class="btn-cancelar" @click.stop="cancelarEstudo(0)">✖</button>
                    </div>
                    <img :src="`/assets/icons/${jogo.processamento[0].item}.png`" class="img-carcaca-mesa">
                    
                    <div class="barra-proc">
                        <div class="fill-proc" :style="{ width: jogo.processamento[0].progresso + '%' }"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="fila-horizontal-container">
        <div class="titulo-fila">Fila de Espera ({{ jogo.processamento.length - 1 }} vagas)</div>
        <div class="grid-fila">
            <div v-for="i in 10" :key="i" class="slot-fila-mini" @click="abrirSelecaoItem(i)">
                <div v-if="jogo.processamento[i] && jogo.processamento[i].item">
                    <img :src="`/assets/icons/${jogo.processamento[i].item}.png`" class="img-mini-fila">
                    <button class="btn-x-mini" @click.stop="cancelarEstudo(i)">x</button>
                </div>
                <div v-else class="vazio-mini">+</div>
            </div>
        </div>
    </div>
    <div v-if="modalFuncionarioAberto" class="modal-overlay" @click.self="modalFuncionarioAberto = false">
        <div class="modal-selecao">
            <div class="modal-header">
                <h3>Selecionar Esfolador</h3>
                <button class="btn-fechar-topo" @click="modalFuncionarioAberto = false">✖</button>
            </div>
            
            <div class="modal-content-scroll">
                <button v-if="jogo.alocacaoCamaraProcessamento[slotFuncionarioSelecionado]" 
    ...
    <div v-if="modalItemAberto" class="modal-overlay" @click.self="modalItemAberto = false">
        <div class="modal-content">
            <h3>Escolha uma carcaça</h3>
            <div v-if="inventarioEstudo.length === 0" class="aviso-vazio">
                Você não possui Carcaças.<br>
            </div>
